...
const CartPreview = ({setViewCart, setViewMenu, setViewmenuOrArticles}) => {
  ...
  const { maxWidth, color1, cart, cartOfCategoryPoints, setCartOfCategoryPoints, } = useContext(AppContext);
  ...
  return (
    <div className={`z-0 d-flex p-3 justify-content-between position-fixed bottom-0 start-50 w-100 rounded-4 rounded-bottom-0 translate-middle-x ${color1.bgColor}`} style={{height:65, maxWidth:maxWidth}} onClick={handleClick}>
